...
public class ProtocolReader(PipeReader pipeReader) : IAsyncDisposable
{
    ...
    public ValueTask<ProtocolReadResult<TReadMessage>> ReadAsync<TReadMessage>(IMessageReader<TReadMessage> reader,
    ...
    public ValueTask<ProtocolReadResult<TReadMessage>> ReadAsync<TReadMessage>(IMessageReader<TReadMessage> reader,
        int maximumMessageSize, CancellationToken cancellationToken = default)
    {
        ...
        int? maximumMessageSize, CancellationToken cancellationToken = default)
    {
        ...
        if (_buffer.Length > 0 && TryParseMessage(maximumMessageSize, reader, _buffer, out var protocolMessage))
        {
            ...
            return new ValueTask<ProtocolReadResult<TReadMessage>>(
                new ProtocolReadResult<TReadMessage>(protocolMessage, _isCanceled, isCompleted: false));
        }
        else
        {
            // We couldn't parse the message so advance the input so we can read
            pipeReader.AdvanceTo(_consumed, _examined);
        ...
        return new ValueTask<ProtocolReadResult<TReadMessage>>(
            new ProtocolReadResult<TReadMessage>(default, _isCanceled, _isCompleted));
    }

    private ValueTask<ProtocolReadResult<TReadMessage>> DoAsyncRead<TReadMessage>(int? maximumMessageSize,
        IMessageReader<TReadMessage> reader, CancellationToken cancellationToken)
    {
        while (true)
        {
            var readTask = pipeReader.ReadAsync(cancellationToken);
            ReadResult result;
            ...
            var (shouldContinue, hasMessage) =
                TrySetMessage(result, maximumMessageSize, reader, out var protocolReadResult);
            if (hasMessage)
            {
    ...
    private async ValueTask<ProtocolReadResult<TReadMessage>> ContinueDoAsyncRead<TReadMessage>(
        ValueTask<ReadResult> readTask, int? maximumMessageSize, IMessageReader<TReadMessage> reader,
    ...
    private (bool ShouldContinue, bool HasMessage) TrySetMessage<TReadMessage>(ReadResult result,
        int? maximumMessageSize, IMessageReader<TReadMessage> reader, out ProtocolReadResult<TReadMessage> readResult)
    {
    ...
    private bool TryParseMessage<TReadMessage>(int? maximumMessageSize, IMessageReader<TReadMessage> reader,
        in ReadOnlySequence<byte> buffer, [MaybeNullWhen(false)] out TReadMessage protocolMessage)
    {
