...
public class MessagePipeReader(PipeReader pipeReader, IMessageReader<ReadOnlySequence<byte>> messageReader)
    : PipeReader
{
    ...
    private bool TryCreateReadResult(ReadResult underlyingReadResult, out ReadResult readResult)
    {
        ...
        if (messageReader.TryParseMessage(buffer, ref _consumed, ref _examined, out _message))
        {
            if (_message.IsEmpty)
            {
